...
typedef struct pipe_control_block{
  Fid_t reader;           // file discriptor for reader
  Fid_t writer;          //  file discriptor for writer
  ...
  char buffer[PIPE_BUFFER_SIZE];      //cycle buffer which will have the data!
...
//invalid operations!
int pipe_invalid_read(void * reader,char* buffer,unsigned int size);
int pipe_invalid_write(void * writer,const char* buffer,unsigned int size);
void* pipe_invalid_open(unsigned int minor);
