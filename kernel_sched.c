...
/*EDITS.........*/
#define MFQ_LEVELS 12
#define MAX_YIELD_TIMES 1000
...
TCB* spawn_thread(PCB* pcb, void (*func)())
{
	...
	tcb->curr_cause = SCHED_IDLE;
	 ...
	 /*EDITS.........*/
 	 tcb->priority= MFQ_LEVELS/2 ;  //init priority and sched_queue_add will add the new thread in the middle queue
 	 tcb->previous_priority = MFQ_LEVELS-1;
 	 tcb -> ptcb_tcb = NULL ;
...
{
	 /* Get the first TCB of the highest priority queue which is not empty  */
  	rlnode * sel =NULL;
	...
	//if all list are empty return current or idle_thread
...
void yield(enum SCHED_CAUSE cause)
{
	...
	/*EDITS.........*/
	restore_priority(current,cause);
	...
	/*EDITS.........*/
  	adjust_priority_SCHED_CAUSE(cause,current);
  	inversion(current);
  	yield_times++;
	if (yield_times>=MAX_YIELD_TIMES)
	{
		boost();
...
/*EDITS.........*/
void adjust_priority_SCHED_CAUSE(enum SCHED_CAUSE cause,TCB *tcb){
	

	if(cause == SCHED_IO && tcb ->priority>0){                  //give highest priority to the user=> SCHED_IO 
		tcb->priority=0;

	}else if (cause == SCHED_QUANTUM && tcb->priority < MFQ_LEVELS-1){      //quantum has expired so decrease priority
		tcb->priority++;
	}
...
/*after a considerable amount of yields we increase priority of all threads => resolve aging*/

void boost(){

  
   for(int i=1;i<MFQ_LEVELS;i++){
       
      for (int j = 0; j<rlist_len(&SCHED[i]); ++j){
         
         rlnode *temp=rlist_pop_front(&SCHED[i]);
         assert(temp!=NULL);
         if(temp->tcb->priority>0){
         	 temp->tcb->priority--;                  //give max priority only to user

         }
        
         rlist_push_back(&SCHED[ temp->tcb->priority],temp);
...
/*Decrease priority of TCB which waits too long with cause = SCHED_MUTEX*/
void inversion(TCB *tcb){

  
  if(tcb->curr_cause==SCHED_MUTEX && tcb->last_cause==SCHED_MUTEX){

    if(tcb->priority>0 && tcb->priority<MFQ_LEVELS-1){
      tcb->previous_priority=tcb->priority;			//save previous priorite to restore it after SCHED_MUTEX has ended!
      tcb->priority++;
...
/* if SCHED_MUTEX has ended restore priority!*/
void restore_priority ( TCB* tcb, enum SCHED_CAUSE cause){
	 if(tcb->curr_cause==SCHED_MUTEX && tcb->last_cause==SCHED_MUTEX && cause != SCHED_MUTEX){
	 	tcb->priority=tcb->previous_priority;
	 	//fprintf(stderr, "restore_priority\n" );
	 }
}
// spawn_ptcb...

// allocate memory and initialize a PTCB
...
// searching method to find ptcb that is conected with a specific thread in CURPROC.
